	...
	public static Specification<RescueCase> withRescueSearchCriteria(RescueSearchCriteria criteria) {
		...
		return (root, query, cb) -> {
			...
			if (criteria.getKeyword() != null) {
				...
				// 組裝WHERE條件 (caseTitle LIKE '%<keyword>%' OR species LIKE '%<keyword>%' OR...)
				// 注意:cb.like()內參數只能放String，因此遇到非字串屬性要先轉換
				Predicate keywordPredicate = cb.or(cb.like(root.get("caseTitle"), "%" + criteria.getKeyword() + "%"),
						cb.like(root.get("species").get("species"), "%" + criteria.getKeyword() + "%"), // 寫species會返回對應的Species實體，等同自動JOINspecies表但仍需再找到需查詢的欄位.get("species")																				
						...
						cb.like(rescueProgressJoin.get("progressDetail"), "%" + criteria.getKeyword() + "%") // 改用rescueProgressJoin
				);
			...
			if (criteria.getTag() != null) {
				predicate = cb.and(predicate, cb.equal(root.get("tag"), criteria.getTag()));
			}

			// 救援狀態
			if (criteria.getRescueStateId() != null) {
				predicate = cb.and(predicate, cb.equal(root.get("caseStateId"), criteria.getRescueStateId()));
			}
			...
			if (criteria.getDistrictAreaId() != null) {
				predicate = cb.and(predicate, cb.equal(root.get("distinctAreaId"), criteria.getDistrictAreaId()));
			}
			...
			if (criteria.getBreedId() != null) {
				predicate = cb.and(predicate, cb.equal(root.get("breedId"), criteria.getBreedId()));
			}
