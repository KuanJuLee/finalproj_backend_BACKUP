	...
	public static Specification<RescueCase> withRescueSearchCriteria(RescueSearchCriteria criteria) {
		...
		return (root, query, cb) -> {
			...
			if (criteria.getKeyword() != null && !criteria.getKeyword().isEmpty()) {
				...
				// 當 root.get("furColor") 為 null 時，SQL 查詢會生成類似 NULL LIKE '%哈哈%'
				// 的條件，這永遠不會匹配任何資料。對於可能為空的欄位，應該在查詢中進行 null 檢查。
				// root.get("species").get("species") 等操作確實等同於自動 JOIN，但如果某些關聯屬性本身為
				// null，則查詢也可能失敗，因為 NULL 無法匹配
				...
				Predicate keywordPredicate = cb.or(
						...
						cb.like(root.get("districtArea").get("districtAreaName"), fuzzyKeyword),
				...
				// query.district(true); // 消除可能因@onetomany又去join導致查詢出相同多筆case，以此可消除重複結果
			...
			if (criteria.getFurColorId() != null) {
				predicate = cb.and(predicate,
						cb.equal(root.get("furColor").get("furColorId"), criteria.getFurColorId()));
			}
