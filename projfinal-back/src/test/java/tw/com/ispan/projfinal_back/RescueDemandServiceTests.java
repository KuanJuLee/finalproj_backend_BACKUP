...
public class RescueDemandServiceTests {
	...
	public void testFrontRescueDemandConvert() {
		...
		List<RescueDemand> rescueDemandList = rescueDemands.stream() // 將rescueDemands這個List<Integer> 轉換成一個
																		// Stream，如此可以對列表中的每個元素進行操作
				.map(rescueDemandRepository::findById) // 對 Stream 中的每個 (整數) 進行映射，呼叫 rescueDemandRepository.findById 方法
				.filter(Optional::isPresent) // findById 會回傳一個 Optional<RescueDemand>，過濾 Stream 中的元素，只保留那些 Optional
												// 內部有值的元素。
